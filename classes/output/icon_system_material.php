...
/**
 * Class allowing different systems for mapping and rendering icons to Material design
 */
class icon_system_material extends icon_system_font {

    /**
     * @var array $map Cached map of moodle icon names to material icon names.
     */
    private $map = [];
    ...
    public function get_icon_name_map() {
        if ($this->map === []) {
            $cache = \cache::make('core', 'materialiconmapping');

            $this->map = $cache->get('mapping');

            if (empty($this->map)) {
                $this->map = $this->get_core_icon_map();
                $callback = 'get_material_icon_map';
    ...
    public function render_pix_icon(renderer_base $output, pix_icon $icon) {
        ...
        return $output->render_from_template('core/pix_icon_material', $data);
    }
